...
def l1_l2_loss(y_true, y_pred, l1_weight, l2_weight):
    ...
    if l1_weight != 0:
        loss += l1_weight*tf.reduce_mean(tf.abs(y_true - y_pred)) 

    if l2_weight != 0:
        loss += l2_weight*tf.reduce_mean(tf.square(y_true - y_pred))

    return loss


def compute_receptive_field_length(dilations, filter_length, target_field_length): 

    half_filter_length = (filter_length-1)/2

    # Due to initial 3 X 1 convolution 
    receptive_field_of_blocks = 2*half_filter_length   

    # Due to dilations 
    receptive_field_of_blocks += 2*half_filter_length*sum(dilations)

    # Include target_field_length
    receptive_field_of_blocks += target_field_length 
   
    # Due to two 3 X 1 convolutions at postprosessing 
    receptive_field = receptive_field_of_blocks + 2*2*half_filter_length

    return receptive_field, receptive_field_of_blocks
...
def tf_linear_to_ulaw(x, u=255.0):
    x = tf.sign(x) * (tf.log(1 + u * tf.abs(x)) / tf.log(1 + u))
    return x
...
def tf_ulaw_to_linear(x, u=255.0):
    y = tf.sign(x) * (1 / u) * (((1 + u) ** tf.abs(x)) - 1)
    return y
...
def read_wav(filename):
    ...
    if audio_signal.dtype != _FLOATX.name: #'float64':
...
def load_wav(wav_path, desired_sample_rate):
    ...
    sequence = sequence.astype(_FLOATX.as_numpy_dtype())
    return sequence
...
'''
def extract_subsequence_with_speech(audio, label, min_length, fs, silence_threshold=0.1):

    indices = get_subsequence_with_speech_indices(audio, min_length, fs, silence_threshold)

    if indices[0] == indices[1]:
        return None, None
    elif label is None:
        return audio[indices[0]:indices[1]], None
    else:    
