...
def get_configuration(learning_phase):
    parser = argparse.ArgumentParser(description='WaveNet argument parser')
    ...
    if learning_phase == 'generation':     
        parser.add_argument('--noisy_speech_filename', type=str, default=None, help='The input noisy filename') 
        parser.add_argument('--label_filename', type=str, default=None, help='The liguistic labels\' filename') 
    ...
    if learning_phase == 'generation':
        if args.model_id is None:
            raise Exception('Please give the model name, which is used to recover a saved model')

        if args.noisy_speech_filename is None:
            raise Exception('Please give the filename of the noisy speech') 

        speaker_name = args.noisy_speech_filename[0:4]
    ...
    if learning_phase == 'generation':
        ...
        cfg['label_filename'] = args.label_filename  
    ...
    if learning_phase == 'generation':
        ...
        cfg['test_label_dir'] = os.path.join(cfg['data_dir'], cfg['test_label_dir'])  
    ...
    cfg['gc_cardinality'] = None
    if cfg['gc_enabled']:
        gc_ids_mapping_filename = os.path.join(cfg['data_dir'], cfg['gc_ids_mapping'])
        fid = open(gc_ids_mapping_filename, 'r')
        lines = fid.readlines() 
        fid.close() 
        cfg['gc_cardinality'] = len(lines)
        
        gc_ids_mapping = {}
        for line in lines:
            words = line.split(' ')
            gc_ids_mapping[words[0]] = int(words[1].rstrip()) 

        cfg['gc_ids_mapping'] = gc_ids_mapping 
             
        if learning_phase == 'generation':
            return cfg, optim_params['learning_rate_method'][cfg['learning_rate_method']], optim_params['optimization_algorithm'][cfg['optimization_algorithm']], gc_ids_mapping.get(speaker_name, 0) 
...
def save_variables(sess, saver, epoch, cfg, model_id): 
    ...
    checkpoint_path = os.path.join(model_path, 'wavenet')
